...
def exercises(muscle_group):
    ...
    if muscle_group == "back":
        ...
        rows = db.execute(
            "SELECT exercises.id, exercises.exercise_name, exercises.type, exercises.body_part, exercises.equipment, exercises.level, images_dataset.description_url, images_dataset.exercise_image1, images_dataset.exercise_image2 FROM exercises JOIN images_dataset ON exercises.exercise_name = images_dataset.exercise_name WHERE body_part = ? AND images_dataset.exercise_image1 IS NOT NULL AND images_dataset.exercise_image1 <> '' AND images_dataset.exercise_image2 <> '' ORDER BY CASE WHEN exercises.level = 'Beginner' THEN 1 WHEN exercises.level = 'Intermediate' THEN 2 WHEN exercises.level = 'Expert' THEN 3 ELSE 4 END, exercises.exercise_name;", muscle_group.capitalize())

    return render_template("exercises.html", active_status_exercises="active", rows=rows, mgroup=mgroup)

# @app.route("/exercises/<muscle_groups/<id>")
# def individual_exercise(muscle_groups, id):
